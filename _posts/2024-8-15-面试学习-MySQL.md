...
- [MySQL](#mysql)
  ...
  - [50.那 ACID 靠什么保证的呢？](#50那-acid-靠什么保证的呢)
    ...
    - [如何保证隔离性？](#如何保证隔离性)
    - [如何保证持久性？](#如何保证持久性)
  - [51.事务的隔离级别有哪些？MySQL 的默认隔离级别是什么？](#51事务的隔离级别有哪些mysql-的默认隔离级别是什么)
...
![InnoDB 回表](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-7d69e289-dc05-47e1-9308-20a8278ebf2e.jpg)
...
如果创建了一个覆盖索引 idx\_users\_name\_email\_age 包含 name、email、age 列：
...
![联合索引](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-e348203c-f00a-42a4-a745-b219d98ea435.jpg)
...
![没有使用 ICP](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-c58f59e0-850b-4dfd-8129-2dfc51cf4768.jpg)

但是，使用了索引下推优化，把 where 的条件放到了引擎层执行，直接根据name like '张%' and age=10的条件进行过滤，减少了回表的次数。

![使用 ICP](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-a8525cf3-2d16-49a9-a7da-a19762ed16df.jpg)

索引条件下推优化可以减少存储引擎查询基础表的次数，也可以减少 MySQL 服务器从存储引擎接收数据的次数。

### 46.MySQL 中有哪几种锁，列举一下？

![MySQL 中的锁](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-a07e4525-ccc1-4287-aec5-ebf3f277857c.jpg)
...
![简化的主键索引](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-013afdbe-889b-4ed0-ae68-c8c9882570d9.jpg)
...
![记录锁](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-8989ac27-e442-4c14-81ad-6bc133d78bfd.jpg)

②、**Gap Lock 间隙锁**

间隙锁(Gap Locks) 的间隙指的是两个记录之间逻辑上尚未填入数据的部分,是一个**左开右开空间**。

![间隙锁](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-d60f3a42-4b0f-4612-b7ad-65191fecb852.jpg)
...
![临键锁](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-ae8a21cc-8b52-467d-9173-4e01b24e04b9.jpg)
...
![插入意向锁](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-751425cb-daba-4da1-bab6-f843254cad3d.jpg)
...
![意向锁](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-31f7f49c-1e5a-4d42-b8b3-e022b3ba82ae.jpg)
...
![事务四大特性](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-eaafb8b8-fbe6-42c0-9cc2-f2e04631b56c.jpg)
...
![ACID 的保证机制](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-20230919103025.png)
...
![事务的四个隔离级别](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-99942529-4a91-420b-9ce2-4149e747f64d.jpg)
...
在 InnoDB 中，每一行数据都有两个隐藏的列：一个是 DB\_TRX\_ID，另一个是 DB\_ROLL\_PTR。

- `DB_TRX_ID`，保存创建这个版本的事务 ID。
- `DB_ROLL_PTR`，指向 undo 日志记录的指针，这个记录包含了该行的前一个版本的信息。通过这个指针，可以访问到该行数据的历史版本。

![版本链](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-20240415084347.png)
...
![update 操作](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-bf4ff00d-01bd-4170-a17b-6919f7873ea4.jpg)
...
![版本链](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-765b3d83-14eb-4b56-8940-9d60bfaf1737.jpg)
...
![ReadView](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-20240415093703.png)
...
![ReadView](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-20240415094939.png)
...
②、如果某个数据版本的 DB\_TRX\_ID 大于 max\_trx\_id，则表示创建该数据版本的事务在生成 ReadView 之后开始，因此对当前事务是不可见的。

③、如果某个数据版本的 DB\_TRX\_ID 在 min\_trx\_id 和 max\_trx\_id 之间，需要判断 DB\_TRX\_ID 是否在 m\_ids 列表中：
...
![读写分离](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-31df767c-db05-4de4-a05b-a45bcf76c1bf.jpg)
...
![业务代码封装](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-771eb01f-3f1a-4437-8e1b-affe4de36ec3.jpg)
...
![数据库中间件](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-f2313613-25bd-4065-8f63-969a4b5757a7.jpg)
...
![主从复制](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-1bfbfcb5-2392-4f98-be1b-a66204da09e5.jpg)
...
![垂直分库](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-2a43af18-617b-4502-b66a-894c2ff4c6c3.jpg)

②、水平分库：按照一定的策略将一个表中的数据拆分到多个库中，例如，按照用户 id 的 hash 值将用户表拆分到不同的库中。

![水平分库](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-debe0fb1-d7f7-4ef2-8c99-13c9377138b6.jpg)
...
![表拆分](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-7cba6ce0-c8bb-4f51-9c3b-e5a44e724c79.jpg)
...
![范围路由](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-b3882ca3-1d04-44e2-9015-7e6c867255a0.jpg)
...
![Hash 路由](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-e01e7757-c337-48c8-95db-2f7cfd2bc036.jpg)
...
![配置路由](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-fcd34332-d38d-455a-875d-d4afd37cac72.jpg)
...
![扩容](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-2d4d94c9-e816-47fc-93dd-a835b1318099.jpg)
...
![扩容](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-5cf01486-72c1-4eab-9f6e-a19c31569f46.jpg)
...
![扩容](https://raw.githubusercontent.com/BugLeesir/image_host01/main/blogs_img/mysql-a122d6d5-fff2-4ccd-8ddb-a9282eb2e2da.jpg)
